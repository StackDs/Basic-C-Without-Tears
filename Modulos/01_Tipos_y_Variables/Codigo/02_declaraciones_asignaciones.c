...
int main(void) {
   ...
   * Todas las variables deben declararse explícitamente antes de usarse.
   * Podemos declarar varias en una línea y darles un valor después.
  ...
  printf("Bandas (Asignacion multiple): IronMaiden = %d, SOAD = %d, Metallica "
         "= %d\n",
  ...
  // ADVERTENCIA INTENCIONAL: Leer una variable automática no inicializada es
  // "Comportamiento Indefinido" (UB) en C. Lo hacemos aquí SOLO para
  // demostrar que C no inicializa variables por ti como Python.
  // Si compilas con -Wall, el compilador emitirá una advertencia.
  int variable_basura;
  printf("\n--- Demostracion de variable sin inicializar ---\n");
  printf("Valor de variable_basura: %d (IMPREDECIBLE - Comportamiento "
         "Indefinido / UB)\n",
         variable_basura);
  ...
  int dividendo = 10;
  int divisor = 3;
  int resto = dividendo % divisor; // Devuelve el resto de la division entera
                                   // (10 / 3 = 3, resto = 1)
  printf("El resto de dividir %d entre %d es: %d\n", dividendo, divisor, resto);

  /** L-values y R-values
   * Un lvalue (left-value) representa un objeto con memoria persistente (ej:
   * una variable 'recipiente'). Un rvalue (right-value) representa un valor
   * temporal o literal (ej: 100). Solo los lvalues pueden estar a la izquierda
   * de un operador de asignacion (=).
   **/
  int recipiente = 100; // 'recipiente' es lvalue, '100' es rvalue.
  // 100 = recipiente; // Esto daria error de compilacion: lvalue required as
  // left operand of assignment
  printf("\nL-value (recipiente) almacena el R-value: %d\n", recipiente);
   ...
   *  Pre incremento: ++x suma uno a la variable antes de usar su valor en una
   * expresion Post incremento: x++ usa el valor actual de la variable y la
   * incrementa despues Como se ve a continuacion
   **/
